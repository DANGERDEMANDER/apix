...
Each tier raises TierUnavailableError if it can't handle the source, and the
...
async def fetch_tier0(url: str, *, timeout: float = 10.0) -> FetchResult:
    ...
    if r.status_code in (403, 429):
        raise TierUnavailableError(f"tier0 blocked: {r.status_code}")
    if len(r.text) < 500:
        raise TierUnavailableError("tier0 body too small; probably SPA")
...
async def fetch_tier1(url: str, *, timeout: float = 15.0) -> FetchResult:
    ...
    if r.status_code >= 400:
        raise TierUnavailableError(f"tier1 http {r.status_code}")

    lower = r.text.lower()
    if "\u20b9" not in r.text and "inr" not in lower and "price" not in lower:
        raise TierUnavailableError("tier1 body has no fare markers")
...
async def fetch_with_escalation(
    url: str,
    *,
    on_tier: TierCallback | None = None,
) -> FetchResult:
